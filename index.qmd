...
fr <- ggplot(france, aes(x = long, y = lat)) +
  geom_polygon(aes(group = group), fill = "lightgrey", color = NA) +
...
cowplot::plot_grid(plotlist = c(fr, sites))
```
...
We can also use the Fourier decomposition to reproject the temperature for different frequencies and illustrate how the Fourier decomposition works.

```{r proj_freq}
#| message: false
#| warning: false
#| fig-cap: HOBO temperature decomposition and reprojection for different frequencies. We used a temperature serie from the 15th to the 20th of May 2023 for a HOBO in Mormal forest measuring air temperature in plot 59_40 with raw data represented in upper left panel. Lower left panel represent the resulting power spectrum after Fourrier's decomposition, with a mean temperature of 1..56°C and frequency above 1/24h highlighted in blue ("trend") and frequency 1/24h and its harmonics highlighted in red ("day"). The resulting temperature projections for the highlighted frequencies are given in the upper right panel (trend in blue) and lower right panel (day in red).
...
data <- "data/data.tsv" %>% 
  ...
  filter(site == "Mormal",
         ...
         plot == "59_40") %>% 
  arrange(datetime) %>% 
  filter(day(datetime) %in% 15:20)
fc <- fft_tab(data$hobo, 24*6) %>% 
...
data$t_day <- fft_reconstruct(fc$coeff_day, fc$frequency[-1], 1:144)+fc$power[1]
g_data <- ggplot(data, aes(datetime, hobo)) +
  geom_line() +
  geom_point() +
  theme_bw() +
...
g_fc <- fc %>% 
  ...
  annotate("text", x=3/24, y=4, label = paste0("Mean:", round(fc$power[1],2)))
g_sup <- ggplot(data, aes(datetime, t_sup)) +
  geom_line(col = cols[3]) +
  theme_bw() +
  xlab("") + ylab("") +
  ylim(3, 22) +
  annotate("text", x=data$datetime[24], y=20, label = "trend", col = cols[3])
g_day <- ggplot(data, aes(datetime, t_day)) +
  geom_line(col = cols[1]) +
  theme_bw() +
  xlab("") + ylab("") +
  ylim(3, 22) +
  annotate("text", x=data$datetime[24], y=20, label = "day", col = cols[1])
...
#| message: false
#| warning: false
#| fig-cap: HOBO and weather station temperature decomposition and reprojection for different frequencies. We used a temperature serie from the 15th to the 20th of May 2023 for a HOBO in Mormal forest measuring air temperature in plot 59_40 with raw data represented in upper left panel. Lower left panel represent the resulting power spectrum after Fourrier's decomposition, with a mean temperature of 1..56°C and frequency above 1/24h highlighted in blue ("trend") and frequency 1/24h and its harmonics highlighted in red ("day"). The resulting temperature projections for the highlighted frequencies are given in the upper right panel (trend in blue) and lower right panel (day in red).
cols <- c("firebrick", "darkgrey",  "darkblue")
data <- "data/data.tsv" %>% 
  read_tsv() %>% 
  filter(year(datetime) == 2023,
